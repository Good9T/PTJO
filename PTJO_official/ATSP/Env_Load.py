...
import torch
from Problem_Load import load_atsp_file
...
class Env:
    ...
    def reset(self):
        ...
        start_node_idx = torch.arange(self.pomo_size)[None, :].expand(self.batch_size, self.pomo_size)
    ...
    def step(self, selected):
        ...
        done = (self.selected_count == self.node_size)
        if done:
    ...
    def _get_cur_dist(self):
        if self.current_node is None:
            return None
        cur_dist = self.problems[self.BATCH_IDX, self.current_node, :1]
        return cur_dist