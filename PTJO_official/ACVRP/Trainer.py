...
class Trainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, epochs + 1):
            ...
            self.result_log.append('train_loss_trans', epoch, loss2)
            ...
            if epoch > 1:
                ...
                util_save_log_image_with_label(image_prefix, self.trainer_params['logging']['log_image_params_2'],
                                               self.result_log, labels=['train_loss_trans'])
    ...
    def _train_one_batch(self, batch_size):
        ...
        trans_prob_list = torch.zeros(batch_size, self.env.pomo_size, 0)
        trans_state, trans_reward, trans_done = self.env.pre_step()
        while not trans_done:
            T_selected, T_prob = self.model(trans_state)
            trans_state, trans_reward, trans_done = self.env.step(T_selected)
            trans_prob_list = torch.cat((trans_prob_list, T_prob[:, :, None]), dim=2)

        trans_log_prob = trans_prob_list.log().sum(dim=2)
        combined_reward = torch.cat([reward, trans_reward], dim=1)
        # (batch , 2*pomo)
        ...
        final_loss_mean = (primal_loss_mean + trans_loss_mean) / 2
