...
from ATSP_Model_LIB import AddAndInstanceNormalization, FeedForward, MixedScoreAttention, _get_encoding, reshape_by_heads
...
class ATSPModel(nn.Module):
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:
            selected = torch.arange(start=0, end=pomo_size)[None, :].expand(batch_size, pomo_size)
...
class ATSP_Decoder(nn.Module):
    ...
    def set_kv(self, encoded_col):
        ...
        self.v = reshape_by_heads(self.Wk(encoded_col), head_num=head_num)
